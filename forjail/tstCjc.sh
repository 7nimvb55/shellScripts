...
ncvlgip(){
	...
	#Function Decimaltobinary Jail Ip

	#Generated adress output in decimal and binary systems
#	echo "$fji1.$fji2.$fji3.$fji4"
#	echo "$fdji1.$fdji2.$fdji3.$fdji4"
	#New Control Virtual Lan Generated IP To Return
...
ncvlsdbip(){
	...
	fdji1=`echo "obase=2;$fji1" | bc | awk -F '|' \
		'{print substr("00000000",1,\
		8-length($1)) $1}'`
	fdji2=`echo "obase=2;$fji2" | bc | awk -F '|' \
		'{print substr("00000000",1,\
		8-length($1)) $1}'`
	fdji3=`echo "obase=2;$fji3" | bc | awk -F '|' \
		'{print substr("00000000",1,\
		8-length($1)) $1}'`
	fdji4=`echo "obase=2;$fji4" | bc | awk -F '|' \
		'{print substr("00000000",1,\
		8-length($1)) $1}'`
	echo "$fdji1.$fdji2.$fdji3.$fdji4"
}
...
# three octets can be changed by user. Last three octets are
# generated randomly.
echo 'Before ncvlgm '>> ./vlscr.out
echo 'Before ncvlgm '
curMAC=$( ncvlgm )
echo 'After ncvlgm '${curMAC} >> ./vlscr.out
echo 'After ncvlgm '${curMAC}
...
#	ngctl connect ${EIFACE}: ${ncbname}: ether link${ncacc}
	...
	if [ $STATIC_ROUTE_CNT != 0 ] ; then
		...
		DEFR_MACADDR=${NCMAC}
#	else
		echo 'Host default route interface '${DEFR_INTERFACE} >> ./vlscr.out
		echo 'Host default route IP '${DEFIP_INTERFACE} >> ./vlscr.out
		echo 'Host default route MAC address '${DEFR_MACADDR} >> ./vlscr.out
	fi
